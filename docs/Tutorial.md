...
- export_spot_embeddings.py：导出每个 spot 的 embedding，支持 `h`（默认）或 `z64`
- visualize_umap_tsne.py：对导出的 `h_*` / `z64_*` embedding 做 UMAP + t-SNE 可视化（SVG）
...
- 模型与结构：
  ...
  - 分类头：--clf_hidden（默认 256,128,64；必须以 64 结尾以保持 `--embed_source z64` 导出/下游兼容）
...
- `--embed_source {h,z64}`：
  ...
  - `z64`：导出 `z64_*` 列，固定 64 维
- 导出 CSV 会额外写入 `embed_source` 与 `embed_dim` 两列，便于后续区分来源和维度
...
说明：
- `--embed_source {h,z64}` 用来指定读取哪类列：
  - `--embed_source h` -> 读取 `h_*`
  - `--embed_source z64` -> 读取 `z64_*`
- 若切换到 `z64`，输出文件名也会随之变为：
  - `umap_tsne_z64_by_tumor.svg`
  - `umap_tsne_z64_by_batch.svg`
  - `umap_tsne_z64_by_cancer.svg`
