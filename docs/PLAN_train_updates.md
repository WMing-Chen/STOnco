# STOnco 训练流程调整方案（Batch Domain + batch_loss_variance + 新指标图 + 外部验证集）
...
### B. batch_loss_variance（总体方差）

**定义（batch 维度）**
- 对每个 epoch 内所有 batch 的 **平均总风险** 记为 `L_i`。
- 平均风险：`avg_total_loss = mean(L_i)`  
- 方差风险（总体方差）：  
  `batch_loss_variance = mean((L_i - avg_total_loss)^2)`

**落地点**
- 在训练循环中记录每个 batch 的 `total_loss`，epoch 结束后计算 `batch_loss_variance`。
- `loss_components.csv` 新增 `batch_loss_variance` 列。
...
3) **batch_loss_variance 列名**
   - 统一为 `batch_loss_variance`

## 命名更新说明

- 2026-04 更新：实现与文档中原先的 `Var_risk/var_risk` 已统一改名为 `batch_loss_variance`。
- 含义不变，仍表示每个 epoch 内 mini-batch 总损失的总体方差；此次更新仅修正命名歧义。
